        ...
        <ul>
          {friendRequest!.map((friendRequest: FriendRequest, index) => {
            return (
              <li key={index}>
                <div>
                  <img src={friendRequest.img} alt={friendRequest.pseudo} />

                  <b>{friendRequest.pseudo}</b>

                  <button onClick={() => acceptRequest(friendRequest.friendship_id, true)}>
                    <IoMdCheckmark />
                  </button>

                  <button
                    onClick={() => acceptRequest(friendRequest.friendship_id, false)}
                  >
                    <RxCross2 />
                  </button>
                </div>
